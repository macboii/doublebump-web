...
| ID | Purpose |
|----|---------|
| `#hero` | Full-viewport opener with phone animation; store buttons plus a `.qr-row` of two App Store / Google Play QR codes, for desktop visitors who want to grab the app on their phone |
...
- **`.qr-row` width sync**: `setLang` also calls `syncQrWidths()`, which sets each `.qr-row`'s width in px to match its sibling `.store-buttons`' rendered width (`getBoundingClientRect()`), also re-run on `window resize`. This is JS, not CSS, because it isn't solvable in CSS here: `.qr-code` needs `width:100%` to be fluid, but a percentage width several levels deep (row → item → img) makes every ancestor's own intrinsic-size query ("how wide do you want to be before I decide my own width") resolve to "fill everything" instead of the buttons row's true content width — true regardless of flex, grid auto-columns, or `display:table`, all of which were tried and discarded before landing here.
