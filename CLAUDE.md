...
- `/ios` and `/android` are cold-outreach tracking redirects (`ios/index.html`, `android/index.html`): read `utm_source`/`utm_medium`/`utm_campaign`/`utm_content` from the query string, fire a GA4 `store_redirect` event (`event_callback` + `event_timeout: 400` + a manual `setTimeout` fallback so an ad-blocked/slow gtag.js never strands the visitor), then redirect to the real App Store / Google Play URL. `/android` also re-encodes the UTM values as a Play Install Referrer string on the destination URL. Both destinations are hardcoded — no open-redirect surface. `noindex, nofollow`, not in `sitemap.xml`. Not app-install attribution, just click measurement.
